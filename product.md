...
- **Tenant isolation** (built, with an honest caveat): every route derives `restaurant_id` from a verified Supabase JWT (`app/auth.py`) — never a client-supplied param — and every query filters on it, including ownership checks on client-supplied ids (`conversation_id`, `chunk_id`, `claim_id`, `card_id`) that would otherwise let one tenant read/mutate another's rows by guessing an id even with auth in place. The RLS policies from §2.2 are written and registered, but the backend still connects with the service-role key, which bypasses them — so RLS isn't yet the thing actually enforcing isolation, the app-layer filtering above is. Enforcing RLS for real (`SET LOCAL request.jwt.claims` per request instead of the service-role bypass) is the documented next hardening step, not yet done.
- **Auth**: Supabase Auth (email/password), with a Postgres custom-access-token hook (`restaurant_members` → `restaurant_id` claim) rather than a hand-rolled login system — the same claim shape the RLS policies were written against from day one.
- **Rate limiting & cost tracking**: per-tenant request caps (`api_requests`, 429 past the limit) and real per-query cost from actual Claude token usage (`query_traces.estimated_cost_usd`), surfaced as an alert-only threshold in Insights — deliberately never blocks a tenant, per the tradeoff of availability over a hard cost ceiling.
...
- **Semantic response cache** (built in Phase 4 as `response_cache`, a `pgvector` table rather than Redis — one fewer moving part, and it was already the retrieval store): a cosine-similarity lookup on the question embedding, scoped to `(restaurant_id, data_version)`. `data_version` is a per-restaurant counter bumped on every successful order or policy-document ingestion, so a whole restaurant's cache invalidates atomically the moment its underlying data changes — no need to know which cached rows are affected, and no TTL to tune. Only `grounded`/`no_claims` verdicts are ever cached; a `partial` or `ungrounded` answer is exactly the kind that must not be served again without re-verification.
- **Embedding cache**: unchanged policy chunks are never re-embedded on subsequent ingestion runs (content-hash keyed). *(Not yet built — the semantic response cache above shipped first since it's the higher-leverage half of "Warmstart.")*
...
### Phase 3 — Diagnostic Intelligence (built)
- **Multi-agent orchestration for multi-hop diagnostic questions** — a new `DIAGNOSTIC` router lane for "why did X change" questions, handled by three chained agents where each step's output decides the next step's shape rather than parallel independent lookups: a trend agent quantifies the change (recent vs. baseline window), a correlation agent finds what's moving alongside it (weather rate, cancellation-reason mix) and picks a likely driver from that, and a policy agent searches for the one clause relevant to that specific driver. Synthesis still runs through the same grounding verification as every other route. Implemented in `multi_agent_investigator.py`.
- **Anomaly-to-Root-Cause Investigator** — `anomaly_scan.py`, invoked via `POST /api/diagnostics/scan` rather than a scheduled worker (consistent with every other proactive capability in this build): computes windowed aggregates per zone and flags a deviation past a percentage threshold with a minimum sample size — a pragmatic stand-in for real change-point detection, documented as such rather than claimed as statistically rigorous. A flagged zone triggers the multi-agent investigator above and writes a fully-cited `diagnosis_cards` row only if grounding passes — an unverified proactive claim is never persisted. This is what separates the feature from commodity threshold alerting: the citation/grounding infrastructure from Phase 2 is what makes an unprompted, autonomous diagnosis trustworthy enough to push.

### Phase 4 — Scale & Optimize (partially built — see note below)
- **Semantic response caching** (built) — `response_cache` (pgvector) keyed on question-embedding similarity within `(restaurant_id, data_version)`; see §2.7. Ingestion bumps `data_version`, invalidating the whole restaurant's cache atomically. Surfaced in the UI as a "served from cache" badge with the match similarity, and in Insights as a cache-hit-rate stat.
- **Observability dashboard** (built) — `GET /api/insights/summary` aggregates real `query_traces`/`response_cache` rows into route distribution, avg latency by pipeline stage, cache hit rate, and a 14-day groundedness trend. Nothing synthetic; it's empty on a fresh install until real queries accumulate.
- **Model cascading tuned with a fine-tuned small router model** (not built) — skipped rather than half-built: this needs real production query volume and training infrastructure to justify and execute, neither of which exists at this build's stage. The few-shot LLM router from Phase 1 already works; revisit this once volume actually makes the training cost worth it.
- **Multi-tenant load testing, read-replica scaling for the SQL engine** (not built) — ops work that needs a live deployment to load-test against; not meaningfully demonstrable as code without one.
- **Service split (router / retrieval / synthesis) behind internal gRPC** (not built) — no second internal service consumes this API yet, so there's nothing for gRPC's typed-contract value to attach to (see §2.3's REST-not-gRPC rationale). Revisit if/when these stages are actually split into independently-scaled services.
- **Cross-Restaurant Benchmark Intelligence** (not built) — once tenant density exists, the nightly cross-tenant materialized view (§2.2) powers a `BENCHMARK` intent class so an operator can see cohort percentiles ("30th percentile on delivery time for your cuisine/zone"). This is a genuine data-network-effect moat — it compounds with every new restaurant onboarded — and opens a second revenue surface: anonymized aggregate insight sold back to the aggregator platforms themselves. Deliberately not built: faking multi-tenant density just to demo the mechanism would misrepresent what it actually does, so it stays deferred until there's real density to benchmark against.
