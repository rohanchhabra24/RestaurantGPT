...
| `grounding_verdict` (`grounded`/`no_claims`/`partial`/`ungrounded`) | Nav badge (`NavBar.jsx`), answer footer (`AnswerCard.jsx`) | "X% verified answers"; per-state tag — "Verified" (grounded) / "No sourcing needed" (no_claims) / "Partially verified" (partial) — each with "N of M facts double-checked against your actual data". `ungrounded` doesn't get a tag at all: see the abstention row below (Stage 2E) — it gets a distinct card, not a badge on a normal-looking answer |
| Stage 2E: an "ungrounded" answer that used to just be the model's own (possibly still confident-sounding) retry text next to a small red "Unverified" tag | Chat, any answer that fails grounding twice (`pipeline.py`'s `_ABSTENTION_MESSAGES`, `AnswerCard.jsx`) | A deterministic, honest "I don't have a confident answer to that" — never model-generated prose — rendered as its own amber-bordered card with a warning icon, visually distinct from a normal answer rather than a normal-looking paragraph with a small tag underneath |
