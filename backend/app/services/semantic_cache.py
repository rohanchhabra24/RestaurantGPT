...
async def lookup(question: str, restaurant_id: str) -> dict | None:
    pool = await get_pool()
    ...
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow(
            """select id, answer_text, citations, data_table, route_taken, grounding_verdict,
               ...
               from response_cache
               where restaurant_id = $2 and data_version = $3
               order by question_embedding <=> $1::vector
               limit 1""",
            vec_literal, rid, current_version,
        )
...
async def store(question: str, restaurant_id: str, result) -> None:
        ...
        await conn.execute(
            """insert into response_cache
               (restaurant_id, question_text, question_embedding, data_version, route_taken,
                answer_text, citations, data_table, grounding_verdict, citation_coverage)
            ...
            result.grounding_verdict, result.citation_coverage,
        )
