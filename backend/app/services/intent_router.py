"""Query routing — the dispatcher deciding SQL vs. retrieval vs. both.

This runs on every single query, so it uses the cheapest model in the
cascade (see product.md §2.5). A wrong route here means everything
downstream is wrong, so the prompt is deliberately narrow: classify and
extract slots, nothing else.
"""
...
async def classify_intent(question: str) -> dict:
    try:
        ...
        if route not in ("SQL", "RETRIEVAL", "HYBRID", "CLARIFY"):
        ...
        if mentions_policy and mentions_data:
            route = "HYBRID"
